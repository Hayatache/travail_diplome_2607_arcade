...
//      CHR 12.04.2016  adaptaion d�tails pour plib_i2c de Harmony 1.06   CHR
//		SCA 04.04.2017  Compl�ments commentaires i2c_init HighFrequencyEnable/Disable
...
void i2c_init( bool Fast )
{
	...
	// (cf. � I2C datasheet PIC32). Toutefois, le LM92 a des probl�mes 
	// d'incompatibilit� avec les flancs trop lents => d�sactiv�
...
// i2c_start()
//
// D�bute la transaction I2C master
//
// Adaptation plib_i2c  : 19.03.2015 CHR
...
// En mode ma�tre, la fonction g�n�re le signal d'horloge, 
//  en mode esclave elle attend le signal d'horloge du ma�tre.
// Param�tre data : la valeur 8 bits � transmettre.
// Valeur de retour : acknowledge donn� par l'autre partie
//  false : l'autre partie n'a pas quittanc� le byte transmis,
//  true  : l'autre partie a quittanc� le byte transmis.
//------------------------------------------------------------------------------
//
...
bool i2c_write( uint8_t data )
{
    ...
    // Wait for the bus to be idle (n�cessaire apr�s un reStart)
    while(PLIB_I2C_BusIsIdle(KIT_I2C_BUS) == false);
